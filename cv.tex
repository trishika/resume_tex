...
\section{\textsc{\T{Experiences}{Expériences}}}
	...
	\subsection{\textsc{\T{Job}{Emplois}}}
		...
		{OpenWide}{Paris}{}
		{\ShortText{
			\T{Work on CCTV image processing software and embedded Linux system developement.}
		...
		\LongText{
			\cvline{}{
				\T{
					...
					In order to maintain the fiability of big CCTV installation, this software checks periodically on the CCTV
					records and computes each camera state. It can detect blur image, over/under-exposition, moved camera...
				}{
					\textbf{Analyse de vidéos surveillance pour la détection de défauts.}
					Afin d'assurer la fiabilité de grands parcs de vidéo surveillance, ce logiciel effectue des contrôles
					sur les caméras du parc et évalues leurs états. Il signale les défauts tel qu'une image floue,
					une sur-exposition ou encore un décadrage de caméra.
				}
				\small
				\begin{itemize}
					...
					\item \T{Developed functionalities}{Listes des fonctionnalités développées} :
						\begin{itemize}
							\item \T{JSON/REST communication with GUI web}{Interfaçage JSON/REST avec l'IHM web} ;
							\item \T{Works on new support of SDK for camera recorder}{Réalisation de supports de SDK d'enregistreurs nécessaires à l'acquisition d'image} ;
							\item \T{Architectural rewrite}{Refonte architecturale lors d'ajout de fonctionnalités dans les méthodes d'acquisitions des images}.
						\end{itemize}
				\end{itemize}
			}
			\cvline{}{
				\T{
					...
					A provided software allowed to control a refrigerated tank. The project aims to build a Linux system and	a QT graphical user interface.
				}{
					\textbf{Carte Linux/ARM de contrôle et de configuration de cuve réfrigérée.}
					À partir d'un code C, réalisé par le client, permettant le contrôle d'une cuve réfrigérée le projet consiste à mettre
				...
				\begin{itemize}
					...
					\item \T{Developed functionalities}{Listes des fonctionnalités développées} :
						\begin{itemize}
							...
							\item \T{C code modifications to interface with GUI}{Modification du code C existant afin de s'interfacer avec l'IHM} ;
							\item \T{Libraries adding to the Linux system with buildroot}{Ajout de librairies dans le système avec Buildroot}.
						\end{itemize}
				\end{itemize}
			}
		}
